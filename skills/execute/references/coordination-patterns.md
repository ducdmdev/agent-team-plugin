...
- [Plan-Mode Coordination](#plan-mode-coordination) — handling PLAN_PROPOSAL messages and approval flow
- [Advanced Patterns](#advanced-patterns)
  - [Re-plan on Block](#re-plan-on-block)
  - [Adversarial Review Rounds](#adversarial-review-rounds)
  - [Quality Gate](#quality-gate)
  - [Checkpoint/Rollback](#checkpointrollback)
  - [Deadline Escalation](#deadline-escalation)
  - [Circular Dependency Detection](#circular-dependency-detection)
  - [Graceful Degradation](#graceful-degradation)
  - [Auto-Block on Repeated Failures](#auto-block-on-repeated-failures)
...
2. **Compile into workspace and report**:
   ...
   - Generate `.agent-team/{team-name}/report.md` from the report format template
...
3. **Enter the Error Recovery Loop** — see [error-recovery-protocol.md](error-recovery-protocol.md) for the full decision tree:
   - Classify the error type (retry, recoverable, design_flaw, unknown)
   - Check the pattern library for known strategies
   - Apply bounded recovery per the decision tree
   - Track all attempts in `issues.md` extended format
4. **If unrecoverable after recovery bounds**:
...
When teammates are spawned with plan-mode active, they send PLAN_PROPOSAL messages before writing code.

### PLAN_PROPOSAL Flow

1. **Teammate sends** `PLAN_PROPOSAL #N` with approach, alternatives, files, complexity, and risks
2. **Lead evaluates** the proposal:
   - Does the approach align with the overall plan?
   - Are the files within the teammate's ownership?
   - Are the alternatives reasonable? Did they consider the right trade-offs?
   - Is the complexity estimate realistic?
3. **Lead responds** with one of:
   - `PLAN_APPROVED #N` — teammate proceeds with implementation
   - `PLAN_REVISION #N: {feedback}` — teammate revises and resubmits
4. **Max 2 revision rounds** — after 2 revisions, the lead decides (approve or provide a directive)
5. **Track** all proposals in `progress.md` Plan Proposals table

### When to Override Plan-Mode

The lead may override plan-mode for a teammate mid-execution:
- **Activate**: "Your remaining tasks now require plan-mode. Send PLAN_PROPOSAL before proceeding."
- **Deactivate**: "Plan-mode no longer required for your tasks. Proceed directly."
- Log overrides in `progress.md` Decision Log
...
> **Note**: The Completion Gate in the audit stage Phase 5 provides mandatory quality checks (build, tests, lint, integration, security, issues, plan completion, doc sync) for ALL teams. This pattern describes extended verification options for complex plans that go beyond the standard gate.
